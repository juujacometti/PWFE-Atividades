...
if (isNaN(dias) || dias < 0 || isNaN(inicio) || inicio < 0 || inicio > 23 || isNaN(fim) || fim < 0 || fim > 23) {
    ...
    if (dias === 0 && fim <= inicio) { // Se não houver dias completos e o fim for menor ou igual ao início, significa que passou da meia-noite
        duracao += 24;
    }
}
...
if (isNaN(horasTrabalhadasMes) || horasTrabalhadasMes <= 0 || isNaN(salarioHora) || salarioHora <= 0) {
    alert("Pelo menos um dos valores informados é inválido! Tente novamente.");
} 

else {
    let cargaMensal = 40 * 4; // 160 horas
    let horasExtras = horasTrabalhadasMes > cargaMensal ? horasTrabalhadasMes - cargaMensal : 0;    // valor se verdadeiro : valor se falso
...
let genero2 = prompt("Certo, agora informe o seu gênero.\nDigite 'F' para Feminino\nDigite 'M' para masculino")
